...
export default function Navbar() {
  ...
  return (
    <header className="bg-white dark:bg-[#121212] border-b border-slate-200 dark:border-white/10 sticky top-0 z-40 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <a
            ...
            className="hidden sm:flex items-center gap-1.5 bg-[#10b981] hover:bg-[#059669] text-white dark:text-[#121212] text-xs font-bold px-3 py-2 rounded-lg transition-colors duration-150"
          >
          ...
          {/* Hamburger */}
          <button
            className="md:hidden text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white p-2 rounded-md hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden bg-white dark:bg-[#121212] border-t border-slate-200 dark:border-white/10 px-4 py-3 flex flex-col gap-1 transition-colors duration-300">
          {NAV_LINKS.map(({ href, label }) => (
            <Link
              key={href}
              href={href}
