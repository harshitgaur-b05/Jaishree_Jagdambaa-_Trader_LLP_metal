...
export function getSingleProductWALink(product) {
  const dim = product.dimension ? ` (${product.dimension})` : '';
  const message = `Hi, I'm interested in ${product.name}${dim}. Please share price and availability.`;
...
export function getCartWALink(cartItems) {
  const lines = cartItems
    .map((item, i) => {
      const dim = item.dimension && item.dimension !== 'undefined' ? ` - ${item.dimension}` : '';
      const type = item.type ? ` [${item.type}]` : '';
      return `${i + 1}. ${item.name}${dim}${type}`;
    })
    .join('\n');
